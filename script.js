...
async function loadPublicProfiles() {
  ...
  try {
    ...
    container.innerHTML = (players || []).map(player => {
      ...
      const recent = rows.slice(0, 5).map(r => `<span class="pill">${escapeHtml(r.week?.title || "Week")} · ${r.total} pts</span>`).join("");
      return `<article class="public-profile-card">
        ...
        <div class="profile-stats"><div><strong>${current?.total ?? 0}</strong><span>This week</span></div><div><strong>${career}</strong><span>Total points</span></div><div><strong>${rows.length}</strong><span>Weeks scored</span></div></div>
        <div class="profile-history">${recent || '<span class="muted">No points recorded yet.</span>'}</div>
      </article>`;
    }).join("");
  } catch (error) {
