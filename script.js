...
async function renderEditor() {
  ...
  if (!teams.length) await loadWeekTeams();
  ...
  profileLibrary.innerHTML = profileList.length
    ? profileList.map(p => `<div class="profile-row"><strong>${escapeHtml(p.full_name || "Unnamed player")}</strong><span>Permanent profile</span></div>`).join("")
  ...
  points.innerHTML = allPlayers.length ? allPlayers.map(p => `<div class="point-row" data-point-player="${escapeHtml(p.id)}"><div><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.teamName)}</small></div><input type="number" min="0" step="1" data-point="batting" placeholder="Batting"><input type="number" min="0" step="1" data-point="bowling" placeholder="Bowling"><input type="number" min="0" step="1" data-point="fielding" placeholder="Fielding"><input type="number" min="0" step="1" data-point="winning" placeholder="Winning"></div>`).join("") : `<div class="notice"><strong>No weekly players assigned.</strong><span>Choose the player profiles for each team above.</span></div>`;
...
async function saveEditorSettings() {
  ...
  try {
    ...
    for (const checkbox of document.querySelectorAll("[data-editor-team]")) {
      ...
      const { error: teamError } = await client.from("week_teams").update({playing: checkbox.checked}).eq("week_id", currentWeek.id).eq("team_id", teamId);
      if (teamError) throw teamError;
    }
    ...
    await loadWeekTeams();
    await renderEditor();
  } catch (error) { console.error(error); editorMessage(error?.message || "Could not save week settings.", true); }
}
