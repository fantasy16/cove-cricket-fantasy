...
async function loadAdminStatus() {
  ...
  const { data, error } = await getSupabaseClient()
    ...
    .maybeSingle();
  if (!error) isAdmin = data?.is_admin === true;
}

function renderEditor() {
  if (!editorNav || !editorSection) return;
  editorNav.classList.toggle("hidden", !currentUser);
  if (!currentUser) editorSection.classList.add("hidden");
  if (!currentUser || !currentWeek) return;
  ...
  if (adminBadge) adminBadge.textContent = isAdmin ? "Admin access" : "Admin access required";
  const box = document.getElementById("editorTeams");
  box.innerHTML = teams.map(team => `
    <label class="check-row"><input type="checkbox" data-editor-team="${team.id}" ${team.playing ? "checked" : ""}> ${escapeHtml(team.name)}</label>
  `).join("");
}
...
async function saveEditorSettings() {
  if (!isAdmin || !currentWeek) {
    ...
    if (message) message.textContent = "Your account does not have Editor/Admin access yet. Run supabase-fix.sql and set profiles.is_admin = true.";
    return;
  }
  const message = document.getElementById("editorMessage");
  ...
  for (const checkbox of checks) {
    ...
    const { error: teamError } = await getSupabaseClient().from("week_teams")
      .update({ playing: checkbox.checked }).eq("week_id", currentWeek.id).eq("team_id", teamId);
    if (teamError) { message.textContent = teamError.message; return; }
  }
  message.textContent = "✅ Week settings saved.";
  await loadWeek();
  await loadWeekTeams();
  renderEditor();
}
