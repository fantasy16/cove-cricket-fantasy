...
async function loadWeek() {
  ...
  if (!data) {
    currentWeek = null;
    document.getElementById("weekNumber").textContent = "No current week";
    document.getElementById("leaderboardWeek").textContent = "No current week";
    document.getElementById("weekStatus").textContent = "The Editor can create or activate a fantasy week.";
    document.getElementById("weekOpen").textContent = "● Not configured";
    document.getElementById("weekOpen").className = "team-off";
    return;
  }
...
async function loadEditorWeeks() {
  if (!isAdmin) { allWeeks = []; return; }
  const { data, error } = await getSupabaseClient()
    .from("weeks")
    .select("id, week_number, title, is_current, selections_open")
    .order("week_number", { ascending: true });
  if (error) throw error;
  allWeeks = data || [];
}

async function ensureInitialWeek() {
  if (!isAdmin || allWeeks.length) return;
  const client = getSupabaseClient();
  const { data, error } = await client.from("weeks").insert({
    week_number: 1, title: "Week 1", is_current: true, selections_open: false
  }).select("*").single();
  if (error) throw error;
  await ensureWeekTeams(data.id);
  await loadWeek();
  await loadWeekTeams();
  await loadEditorWeeks();
}
...
function renderEditor() {
  ...
  const currentInputsDisabled = !currentWeek;
  document.getElementById("editWeekNumber").disabled = currentInputsDisabled;
  document.getElementById("editWeekTitle").disabled = currentInputsDisabled;
  document.getElementById("editSelectionsOpen").disabled = currentInputsDisabled;
  document.getElementById("saveEditor").disabled = currentInputsDisabled;

  document.getElementById("editWeekNumber").value = currentWeek?.week_number ?? "";
  ...
  const weekSelect = document.getElementById("switchWeekSelect");
  if (weekSelect) {
    weekSelect.innerHTML = allWeeks.length
      ? allWeeks.map(w => `<option value="${escapeHtml(w.id)}" ${currentWeek?.id === w.id ? "selected" : ""}>Week ${escapeHtml(w.week_number)} — ${escapeHtml(w.title || "Untitled")}${w.is_current ? " (current)" : ""}</option>`).join("")
      : `<option value="">No weeks created yet</option>`;
  }
  ...
  teamChecks.innerHTML = currentWeek && teams.length
    ? teams.map(team => `
    ...
    `).join("")
    : `<div class="notice"><strong>${currentWeek ? "No team slots exist for this week." : "No current fantasy week is active."}</strong><span>${currentWeek ? "They will be created automatically when this week is initialized." : "Choose an existing week below or start a new week."}</span></div>`;
  ...
  teamSelect.disabled = !currentWeek || !teams.length;

  const roster = document.getElementById("editorRoster");
  roster.innerHTML = currentWeek ? teams.map(team => `
    <div class="roster-group">
      <strong>${escapeHtml(team.name)}</strong>
      ${team.players.length ? team.players.map(p => `<span>${escapeHtml(p.name)}</span>`).join("") : `<span class="muted">No players yet</span>`}
    </div>
  `).join("") : `<div class="notice"><strong>No current week selected.</strong><span>Select a week above to view its roster.</span></div>`;

  const points = document.getElementById("editorPoints");
  const allPlayers = currentWeek ? teams.flatMap(team => team.players.map(p => ({...p, teamName: team.name}))) : [];
...
async function switchCurrentWeek() {
  if (!isAdmin) return editorMessage("You need Admin access to switch weeks.", true);
  const weekId = document.getElementById("switchWeekSelect")?.value;
  if (!weekId) return editorMessage("Choose a week first.", true);
  editorMessage("Switching current week…");
  try {
    const client = getSupabaseClient();
    const { error: offError } = await client.from("weeks").update({ is_current: false }).eq("is_current", true);
    if (offError) throw offError;
    const { error: onError } = await client.from("weeks").update({ is_current: true }).eq("id", weekId);
    ...
    render();
    renderEditor();
    editorMessage(`✅ Week ${currentWeek.week_number} is now the current week.`);
  } catch (error) {
    console.error(error);
    editorMessage(error?.message || "Could not switch weeks.", true);
  }
}
...
document.getElementById("switchWeekBtn").addEventListener("click", switchCurrentWeek);
