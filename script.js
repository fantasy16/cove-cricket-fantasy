...
async function saveTeam() {
  ...
  if (fantasyTeamId) {
    const { error } = await supabaseClient
      .from("fantasy_teams")
      .update({ captain_player_id: fantasyCaptain, updated_at: new Date().toISOString() })
      .eq("id", fantasyTeamId);
...
document.getElementById("loginBtn").addEventListener("click", () => {
  if (currentUser) {
    if (confirm("Log out of Cove Cricket Fantasy?")) {
      supabaseClient.auth.signOut().then(async () => {
        currentUser = null;
        selected.clear();
        fantasyCaptain = null;
